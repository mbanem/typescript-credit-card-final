...
export const CardNumber: React.FC<ICardNumber> = (
	...
	// useEffect(() => { 
	// 	if (cardMaskState.caretPos != null){
	// 		setPos(cardMaskState.caretPos)
	// 	} else {
	// 		console.log('pos', pos)
	// 	}
	// }, [cardMaskState])
	...
	const equal = (ix: number) => {
		console.log('maskPos, selPos, delta', maskPos, p, caretState.cardNumber.delta)
		return ix===maskPos
	}
	// maskIx show what char is rendered in a label including group-space separator
	// we reduce maskIx for number of preceding spaces and that number should be
	// equivalent to input box caret position,and when tey natch we return true
	// let isHighlighted = false
	// const shouldHighlight = (maskIx: number) => {
	// 	if (isHighlighted) {
	// 		return false
	// 	}
	...
	// 	if (p === maskIx) {
	// 		isHighlighted = true
	// 		console.log('cardMaskState', cardMaskState)
	// 		cardMaskState.forEach(el =>   console.log('el', `<${el}>`) )
	// 	}
	// 	return p === maskIx
	// }
